...
ScrollTrigger.normalizeScroll({ target: scrollWrapper, momentum: self => Math.min(3, self.velocityY / 1000) });
...
function drawWires(time) {
  ...
  const delayedScrollFrac = Math.max(0, (scrollFrac - WIRE_SCROLL_DELAY) / (1 - WIRE_SCROLL_DELAY));

  const maxWireDocY = WIRE_ORIGIN_DOC_Y + 1.5 * (2 / WIRE_FREQ);
  const revealFrac = Math.min(wireState.progress * WIRE_INITIAL_REACH + delayedScrollFrac * 0.8, maxWireDocY);
