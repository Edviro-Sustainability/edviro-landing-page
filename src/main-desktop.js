...
let maskComposer = null;
let wireBloomPass = null;

{
  ...
  wireBloomPass = new UnrealBloomPass(
...
const schoolNoiseUniforms = {
  uMaskTexture: { value: maskComposer ? maskComposer.readBuffer.texture : null }, uResolution: { value: renderResolution },
...
function applyTheme() {
  ...
  if (wireBloomPass) wireBloomPass.strength = theme.wireBloomStrength;
...
if (sceneCards.length === 3) {
  ...
  const cardStarts = [0.46, 0.59, 0.72].map(v => v * CAMERA_SECTION_SCALE);
...
window.addEventListener('resize', () => {
  ...
  if (dofPass) dofPass.setSize(window.innerWidth, window.innerHeight);
  if (wireBloomPass) wireBloomPass.setSize(window.innerWidth, window.innerHeight);
  if (fxaaPass) fxaaPass.material.uniforms.resolution.value.set(1 / (window.innerWidth * pixelRatio), 1 / (window.innerHeight * pixelRatio));
  for (const outlineMaterial of schoolOutlineMaterials) {
    outlineMaterial.resolution.set(window.innerWidth, window.innerHeight);
  }
